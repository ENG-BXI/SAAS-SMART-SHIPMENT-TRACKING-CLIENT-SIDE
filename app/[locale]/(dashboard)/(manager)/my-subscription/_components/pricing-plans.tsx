          ...
          <div key={plan.id} className={`relative bg-white rounded-[28px] border flex flex-col justify-between transition-all duration-300 ${isCurrent ? 'border-custom-primary-color shadow-[0_20px_60px_-35px_rgba(27,131,84,0.9)] ring-2 ring-custom-primary-color/10' : 'border-gray-200 shadow-sm hover:shadow-md'}`}>
            ...
            {isCurrent && (
              <div className='absolute -top-3 left-1/2 -translate-x-1/2'>
                <Badge className='bg-gray-900 text-white text-xs px-3.5 py-0.5 font-semibold rounded-full'>{t('currentPlan')}</Badge>
              </div>
            )}
            {/* Header */}
            <div className='p-6 pb-3'>
              <div className='flex items-center justify-between gap-4'>
                ...
                <Badge className='text-xs font-semibold rounded-full px-3 py-0.5 border bg-gray-100 text-slate-700'>{isBest ? t('bestValue') : '—'}</Badge>
              </div>
              <p className='mt-4 text-sm leading-6 text-slate-600 min-h-18'>{t('description')}</p>
              {/* Pricing info */}
              <div className='mt-6 rounded-[24px] border border-gray-100 bg-[#F7FDF4] px-4 py-4'>
                ...
                <div className='mt-2 flex items-center justify-between text-sm text-slate-700'>
                  <span>{t('billing')}</span>
                  <span className='font-semibold text-slate-900'>{isYearly ? t('yearly') : t('monthly')}</span>
                </div>
                <div className='mt-2 flex items-center justify-between text-sm text-slate-700'>
