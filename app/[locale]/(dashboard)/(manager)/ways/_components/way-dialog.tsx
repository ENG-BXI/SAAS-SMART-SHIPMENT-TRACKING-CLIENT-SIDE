...
import PickerLocationMap from './picker-location-map';
...
function WayDialog(props: WayDialogProps) {
  ...
  const showMapLocation = watch('showMapLocation');
  const token = cookie.get('token');
  useEffect(() => {
    if (!showMapLocation) {
      const points = getValues('points');
      const cleaned = points.map(({lat, lng, ...rest}) => rest);
      setValue('points', cleaned, {
        shouldDirty: true,
        shouldValidate: true
      });
    }
  }, [getValues, setValue, showMapLocation]);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className='md:min-w-150 max-w-3xl'>
        <DialogHeader>
            ...
            <FieldGroup className='gap-y-2'>
              <div className='flex flex-col gap-2 items-center'>
                ...
                <Controller
                  ...
                  render={({field}) => (
                    <Field className='flex flex-row items-center justify-between gap-4 rounded-lg px-3 py-2 bg-muted/30'>
                      <FieldLabel htmlFor={field.name} className='flex flex-col items-start text-sm font-medium cursor-pointer'>
                        {t('fields.locationTitle')}
                        <span className='text-xs text-muted-foreground'>{t('fields.locationDescription')}</span>
                      </FieldLabel>
                      <Switch dir='ltr' id={field.name} checked={field.value} onCheckedChange={field.onChange} className='w-11 h-6' />
                    </Field>
                  )}
                />
              </div>
              ...
              {errors.points?.root?.message && <p className='text-red-500 text-sm'>{errors.points.root.message}</p>}
