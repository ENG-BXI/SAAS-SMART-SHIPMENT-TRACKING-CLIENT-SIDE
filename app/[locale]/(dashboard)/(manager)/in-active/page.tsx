...
const InactivePage = async () => {
  const t = await getTranslations('inactivePage');
  return (
    <div className='w-full pb-10 font-sans'>
      <PageDashboardHeader
        ...
        breadcrumbList={[
          ...
          {text: t('breadcrumb.inactive'), path: '/in-active'}
        ]}
      />
      ...
      <div className='grid gap-6 xl:grid-cols-[1.35fr_0.9fr]'>
        <Card className='overflow-hidden'>
          ...
          <CardContent className='space-y-6'>
            ...
            <div className='space-y-4 text-slate-700'>
              <p className='text-base font-semibold text-slate-900'>{t('card.reasonsTitle')}</p>
              <ul className='space-y-3 list-inside list-disc text-sm leading-7'>
                <li>{t('card.reasons.reason1')}</li>
                <li>{t('card.reasons.reason2')}</li>
                <li>{t('card.reasons.reason3')}</li>
              </ul>
            </div>
        ...
        <Card className='overflow-hidden'>
          ...
          <CardContent className='grid gap-4 text-sm text-slate-700'>
            ...
            <div className='rounded-3xl border border-slate-200 bg-white p-5'>
              <p className='font-semibold text-slate-900'>{t('statusCard.nextSteps.title')}</p>
              <p className='mt-2'>{t('statusCard.nextSteps.text')}</p>
            </div>
          </CardContent>
        </Card>
      </div>
