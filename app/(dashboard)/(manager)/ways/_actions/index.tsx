...
export async function UpdateWays(id: string, token: string, data: wayFormData) {
  try {
    const response = await serverAxiosInstance.put(`${WAY}/${id}`, data, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    });
...
export async function DeleteWay(id: string, token: string) {
  try {
    const response = await serverAxiosInstance.delete(`${WAY}/${id}`, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    });
